...
uartinit(void)
{
  ...
  /* Note:
   * INT_TX_ENABLE置1后, 如果 UART 的发送 FIFO 空闲（即可以接受新的数据），会触发发送中断。
   *
   * 持续触发uart中断的原因:
   * UART 的中断配置 (IMSC) 启用了接收和发送中断。
   * 如果 UART 的接收 FIFO 或发送 FIFO 状态满足中断条件（如接收 FIFO 非空或发送 FIFO 空闲），中断会持续触发!
   *
   * 若将 "WriteReg(IMSC, INT_RX_ENABLE | INT_TX_ENABLE)" 修改为 "WriteReg(IMSC, INT_RX_ENABLE)",
   * 就不会在没有键盘敲击产生的中断情况下, 持续触发uart中断了！
   */
